...
<div class="space-y-8">
    ...
    @else
            ...
            <div class="space-y-4">
                ...
                @if (count($orphanageCrs) > 0)
                    <div class="flex justify-between gap-4 items-center">
                        ...
                        <select id="sort_orphanageCr" name="sort_orphanageCr" wire:model="orphanageCrDropdownSort"
                            class="dropdown w-fit rounded-md shadow-sm pl-3 pr-10 font-medium border-transparent focus:border-transparent bg-blue-500 text-white focus:ring focus:ring-blue-500 focus:ring-opacity-50 cursor-pointer">
                            @foreach ($orphanageCrs as $item)
                                <option value="{{ $item['id']}}"
                                    {{ $orphanageCrDropdownSort == $item['id'] ? 'selected' : null }}>
                                    {{ $item['name'] }}
                                </option>
                            @endforeach
                        </select>
                            ...
                            <tbody>
                                @foreach ($orphanCrs as $index => $item)
                                    <tr class="odd:bg-white even:bg-gray-100">
                                        ...
                                        <td class="whitespace-nowrap px-3 py-4">
                                            <a
                                                href="{{ route('detail-user', App\Models\Orphan::find($item['orphan_id'])->orphanage_id) }}">
        ...
        @else
            <p>Maaf, Anda belum bisa memberikan rekomendasi perlombaan kepada Anak Panti. Rekomendasi hanya
                dapat
