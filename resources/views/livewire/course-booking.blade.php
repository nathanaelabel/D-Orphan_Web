    ...
    @else
        <div class="flex flex-col space-y-4">
            <div class="grid grid-cols-2 gap-4 md:grid-cols-2 lg:grid-cols-4">
                <x-dasbor.card>
                    <x-slot:title>Pendapatan bulan ini</x-slot:title>
                    <x-slot:icon>
                        <path stroke-linecap="round" stroke-linejoin="round"
                            d="M4.5 19.5l15-15m0 0H8.25m11.25 0v11.25" />
                    </x-slot:icon>
                ...
                <x-dasbor.card>
                    ...
                    <x-slot:number>
                        {{ App\Models\CourseBooking::whereIn('course_id', App\Models\Course::where('tutor_id', Auth::user()->tutor->id)->pluck('id'))->where('status', 'pending')->count() }}
                    </x-slot:number>
                </x-dasbor.card>
                <x-dasbor.card>
                    <x-slot:title>Kursus sedang berlangsung</x-slot:title>
                    ...
                    <x-slot:number>
                        {{ App\Models\CourseBooking::whereIn('course_id', App\Models\Course::where('tutor_id', Auth::user()->tutor->id)->pluck('id'))->where('status', 'ongoing')->count() }}
                    </x-slot:number>
                </x-dasbor.card>
                <x-dasbor.card>
                    <x-slot:title>Kursus telah diselesaikan</x-slot:title>
                    ...
                    <x-slot:number>
                        {{ App\Models\CourseBooking::whereIn('course_id', App\Models\Course::where('tutor_id', Auth::user()->tutor->id)->pluck('id'))->where('status', 'complete')->count() }}
                    </x-slot:number>
                </x-dasbor.card>
            </div>
        </div>
