<div>
    ...
    {{-- Form Ubah Peserta Kursus atau Anak Panti --}}
    @if (isset($orphan))
        <form wire:submit.prevent="updateOrphan({{ $orphan->id }})">
            <div class="form-group">
                <label for="name">Name</label>
                <input type="text" class="form-control" wire:model="name">
            </div>
    ...
    {{-- Tabel Peserta Kursus atau Anak Panti --}}
    <table class="table mt-4">
        <button wire:click="createOrphan" class="btn btn-sm btn-success mb-4">Create Orphan</button>
        <thead>
            <tr>
                <th>Name</th>
                <th>Date of Birth</th>
        ...
        <tbody>
            @foreach ($orphans as $orphan)
                <tr>
                    ...
                    <td>{{ $orphan->note }}</td>
                    <td>
                        <button wire:click="updateOrphan({{ $orphan->id }})"
